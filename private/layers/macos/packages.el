...
(setq macos-packages
      '(
        ...
        helm
        launchctl
        (osx-dictionary :toggle macos-use-dictionary-app)
        osx-trash
        pbcopy
...
(defun macos/post-init-exec-path-from-shell ()
    ...
    (let ((gls (executable-find "gls")))
      (when gls
        (setq insert-directory-program gls
              dired-listing-switches "-ahlv --group-directories-first --color")))))

(defun macos/pre-init-helm ()
...
(defun macos/init-osx-dictionary ()
  (use-package osx-dictionary
    :if macos-use-dictionary-app
